...
echo Apagando banco de dados (producao)...
del /f /q "%USERDATA%\data\lumina.db" 2>nul && echo   [OK] lumina.db removido || echo   [--] lumina.db nao encontrado

echo Apagando banco de dados (dev - sentinel.db)...
del /f /q "%~dp0..\data\sentinel.db" 2>nul && echo   [OK] sentinel.db removido || echo   [--] sentinel.db nao encontrado

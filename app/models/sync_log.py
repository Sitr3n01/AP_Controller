...
class SyncLog(Base):
    ...
    sync_type: Mapped[SyncType] = mapped_column(
        SQLEnum(SyncType), nullable=False, default=SyncType.ICAL, comment="Tipo de sincronização"
    )

    status: Mapped[SyncStatus] = mapped_column(SQLEnum(SyncStatus), nullable=False, comment="Status da sincronização")

    # Estatísticas
    bookings_added: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Número de reservas adicionadas"
    )

    bookings_updated: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Número de reservas atualizadas"
    )

    bookings_cancelled: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Número de reservas canceladas"
    )

    conflicts_detected: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Número de conflitos detectados"
    )

    # Detalhes técnicos
    error_message: Mapped[str | None] = mapped_column(Text, nullable=True, comment="Mensagem de erro (se houver)")

    sync_duration_ms: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Duração da sincronização em milissegundos"
    )
    ...
    completed_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True, comment="Fim da sincronização")

    # Relacionamentos
    calendar_source: Mapped["CalendarSource"] = relationship("CalendarSource", lazy="selectin")

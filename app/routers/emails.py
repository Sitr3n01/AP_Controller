...
def get_email_service() -> EmailService:
    ...
    if settings.EMAIL_PROVIDER.lower() in ["gmail", "outlook", "yahoo"]:
        return EmailService.from_provider(
            provider=settings.EMAIL_PROVIDER.lower(), username=settings.EMAIL_FROM, password=settings.EMAIL_PASSWORD
        )
...
async def send_email(
    ...
    try:
        ...
        return EmailResponse(success=True, message="Email enviado com sucesso", message_id=result.get("message_id"))
...
async def send_booking_confirmation(
    ...
    if not booking:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Reserva {booking_request.booking_id} não encontrada"
        )
    ...
    if not guest_email:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Hóspede não possui email cadastrado")
    ...
    if send_in_background:
        ...
        return EmailResponse(success=True, message="Email de confirmação será enviado em background")
    else:
        await send_confirmation()
        return EmailResponse(success=True, message="Email de confirmação enviado com sucesso")
...
async def send_checkin_reminder(
    ...
    if not booking:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Reserva {reminder_request.booking_id} não encontrada"
        )
    ...
    if send_in_background:
        ...
        return EmailResponse(success=True, message="Email de lembrete será enviado em background")
    else:
        await send_reminder()
        return EmailResponse(success=True, message="Email de lembrete enviado com sucesso")
...
async def send_bulk_checkin_reminders(
    ...
    bookings = (
        db.query(Booking)
        .options(joinedload(Booking.property_rel), joinedload(Booking.guest))
        .filter(Booking.check_in_date == target_date, Booking.status == BookingStatus.CONFIRMED)
        .limit(batch_size)
        .all()
    )  # FIX: Limitar quantidade

    if not bookings:
        return EmailResponse(
            success=True, message=f"Nenhuma reserva encontrada para check-in em {target_date.strftime('%d/%m/%Y')}"
        )
    ...
    async def send_all_reminders():
        ...
        results = await asyncio.gather(*[limited_send(bk) for bk in bookings], return_exceptions=True)
    ...
    return EmailResponse(success=True, message=f"Enviando lembretes para {len(bookings)} reservas em background")
...
async def fetch_emails(
    ...
    try:
        ...
        result = await email_service.fetch_emails(
            folder=safe_folder, limit=request.limit, unread_only=request.unread_only
        )
        ...
        return EmailListResponse(success=True, total=len(emails), emails=[EmailItem(**email) for email in emails])
...
async def test_email_connection(
    email_service: EmailService = Depends(get_email_service), current_user: User = Depends(get_current_active_user)
):
    """

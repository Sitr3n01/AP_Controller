...
class AIPricingService:
    ...
    def _get_historical_bookings(self, property_id: int):
        ...
        history = (
            self.db.execute(
                select(Booking)
                .where(and_(Booking.property_id == property_id, Booking.status != "cancelled"))
    ...
    def get_pricing_suggestions(self, property_id: int):
        ...
        try:
            ...
            response = self.client.messages.create(
                model=anthropic_model, max_tokens=2000, temperature=0.3, messages=[{"role": "user", "content": prompt}]
            )
